...
{
    if (self.hidden || [[[self class] classesToSkipAccessibilitySearchRecursion] containsObject:[self class]])
    {
        return nil;
    }
    ...
    while (elementStack.count > 0)
    {
        ...
        {
            // If the view is an accessibility container, and we didn't match,
    ...
    for (UIView* view in self.subviews)
    {
        ...
        if (matching.count > 0)
        {
            [matches addObjectsFromArray:matching];
        }
    }
    
    return [[matches copy] autorelease];
}
